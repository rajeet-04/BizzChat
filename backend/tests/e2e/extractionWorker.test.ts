 ...
 *   Enqueue job → Worker picks it up → Gemini SDK is mocked →
 ...
 *   • @google/generative-ai SDK — mocked with vi.mock (no real API calls)
 *
...
//
// vi.hoisted() lifts the mockGenerateContent ref ABOVE vi.mock() hoisting so
// the same function reference is visible both inside the factory and in test
// bodies. This lets individual tests configure responses with
// mockResolvedValueOnce / mockRejectedValueOnce without touching the module
// boundary.
// ---------------------------------------------------------------------------

const { mockGenerateContent } = vi.hoisted(() => ({
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  mockGenerateContent: vi.fn<any, any>(),
}));
...
/** Canonical happy-path Gemini response reused across tests. */
const buildAiResponse = (args = MOCK_AI_RESPONSE) => ({
  response: {
    candidates: [
      {
        content: {
    ...
    usageMetadata: { promptTokenCount: 350, candidatesTokenCount: 120 },
  },
});
...
describe("Extraction Worker E2E", () => {
  beforeEach(async () => {
    ...
    // call mockGenerateContent.mockResolvedValueOnce(...) or mockRejectedValueOnce(...).
    mockGenerateContent.mockReset();
    mockGenerateContent.mockResolvedValue(buildAiResponse());
  });
  ...
  it("processes a single_message job and persists the order", async () => {
    ...
    mockGenerateContent.mockResolvedValueOnce(buildAiResponse(SINGLE_MSG_RESPONSE));
  ...
  it("calls the Gemini SDK exactly once per job", async () => {
    const messages: schema.ChatMessage[] = [
