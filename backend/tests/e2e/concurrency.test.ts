...
//
// vi.hoisted() ensures mockGenerateContent is defined before vi.mock() is
// evaluated, so tests can reconfigure the mock on a per-call basis.
// ---------------------------------------------------------------------------
...
const buildAiResponse = (args = MOCK_ORDER_RESPONSE) => ({
  response: {
    candidates: [
      {
        content: {
...
async function extractionProcessor(
  ...
  const order = await geminiService.extractOrderFromChat(job.data.messages);
...
describe(`BullMQ Concurrency — worker concurrency limit (max ${WORKER_CONCURRENCY})`, () => {
  it(`never exceeds ${WORKER_CONCURRENCY} simultaneous Gemini calls`, async () => {
    let activeAiCalls = 0;
    ...
    expect(mockGenerateContent).toHaveBeenCalledTimes(CONCURRENCY_JOB_COUNT);
