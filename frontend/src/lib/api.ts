...
import type { Order, Stats, QueueHealth, HealthCheck, AsyncJobResponse, JobStatus, Invoice, InventoryResponse, CatalogResponse } from "./types";
...
export class ApiClient {
  ...
  getCatalog = () => this.request<CatalogResponse>("/api/catalog");

  updateCatalogPrice = (name: string, price: number, unit?: string) =>
    this.request<{ success: boolean; name: string; price: number }>("/api/catalog/price", {
      method: "POST",
  ...
  extractMessage = async (message: string): Promise<Order> => {
    const raw = await this.request<any>("/api/extract", {
      method: "POST",
      body: JSON.stringify({ message }),
    });

    // Normalize ExtractedOrder → Order so the UI fields align
    // Backend returns: { name, quantity, unit, pricePerUnit, totalPrice }
    // Frontend Order expects: { product_name, quantity, price }
    const normalized: Order = {
      ...
      items: (raw.items ?? []).map((item: any) => ({
        ...
        price: item.totalPrice ?? item.pricePerUnit ?? item.price ?? null,
        unit: item.unit ?? null,
      })),
      delivery_address: raw.deliveryAddress ?? null,
      delivery_date: raw.deliveryDate ?? null,
      special_instructions: raw.notes ?? null,
      total: raw.totalAmount ?? null,
      confidence: raw.confidence >= 0.7 ? "high" : raw.confidence >= 0.4 ? "medium" : "low",
  ...
  generateInvoice = (orderId: string) =>
    this.request<{ message: string; invoice: Invoice; downloadEndpoint: string }>("/api/orders/invoice", {
      method: "POST",
