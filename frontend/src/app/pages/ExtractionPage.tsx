...
/* ─── Edit Order Modal ─── */
function EditOrderModal({ order, onClose, onSave }: { order: Order, onClose: () => void, onSave: (updates: any) => Promise<void> }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.4)" }}>
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: "#E5E7EB" }}>
          <h2 className="text-lg font-bold text-gray-900 m-0 font-['DM_Sans']">Edit & Confirm Order</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600">×</button>
        </div>
        
        <div className="px-6 py-4 overflow-y-auto flex-1 flex flex-col gap-4">
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">Customer Name</label>
            <input 
              value={form.customer_name} 
              onChange={e => setForm({...form, customer_name: e.target.value})} 
              className="w-full border rounded px-3 py-2 text-sm focus:outline-blue-500 text-gray-900" 
            />
          </div>

          <div>
            <label className="block text-xs font-medium text-gray-500 mb-2 uppercase tracking-wide">Line Items</label>
            <div className="flex flex-col gap-2">
              {form.items.map((item, id) => (
                <div key={id} className="grid grid-cols-[2fr_1fr_1fr_1.5fr] gap-2 items-center bg-gray-50 p-2 rounded">
                  <input 
                    value={item.product_name} 
                    onChange={e => {
                      const newItems = [...form.items];
                      newItems[id].product_name = e.target.value;
                      setForm({...form, items: newItems});
                    }}
                    placeholder="Item name"
                    className="w-full border rounded px-2 py-1.5 text-sm focus:outline-blue-500 text-gray-900" 
                  />
                  <input 
                    value={item.quantity} 
                    type="number"
                    onChange={e => {
                      const newItems = [...form.items];
                      newItems[id].quantity = Number(e.target.value);
                      setForm({...form, items: newItems});
                    }}
                    placeholder="Qty"
                    className="w-full border rounded px-2 py-1.5 text-sm focus:outline-blue-500 text-gray-900" 
                  ...
                  <input 
                    ...
                    onChange={e => {
                      const newItems = [...form.items];
                      newItems[id].unit = e.target.value;
                      setForm({...form, items: newItems});
                    }}
                    placeholder="Unit"
                    className="w-full border rounded px-2 py-1.5 text-sm focus:outline-blue-500 text-gray-900" 
                  ...
                  <input 
                    ...
                    onChange={e => {
                      const newItems = [...form.items];
                      newItems[id].price = Number(e.target.value);
                      setForm({...form, items: newItems});
                    }}
                    placeholder="₹ Price"
                    className="w-full border rounded px-2 py-1.5 text-sm focus:outline-blue-500 text-gray-900" 
            ...
            <button 
              onClick={() => setForm({...form, items: [...form.items, { product_name: "", quantity: 1, unit: "pcs", price: 0 }]})}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">Delivery Info</label>
            <input 
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">Special Instructions</label>
            <textarea 
              value={form.special_instructions} 
              onChange={e => setForm({...form, special_instructions: e.target.value})} 
              className="w-full border rounded px-3 py-2 text-sm focus:outline-blue-500 text-gray-900 resize-none h-16" 
            />
          </div>
        </div>

        <div className="px-6 py-4 border-t bg-gray-50 flex justify-end gap-3" style={{ borderColor: "#E5E7EB" }}>
          <button 
            onClick={onClose}
...
export function ExtractionPage() {
  ...
  const EXAMPLE_MSG =
    "xibit bhai kal 5 kilo chawal aur 2 litre tel dena, evening time mein";
  ...
  return (
    ...
    >
          ...
          >
            <button
              onClick={async () => {
                ...
                try {
                  const res = await client.generateInvoice(extractedOrder.id);
                  window.open(res.downloadEndpoint, "_blank");
                  setExtractedOrder({ ...extractedOrder, invoice: res.invoice });
                } catch (err: any) {
              ...
              style={{
                ...
                fontWeight: 600,
                color: "#FFFFFF",
                backgroundColor: extractedOrder?.invoice ? "#00C853" : "#1A1A2E",
                borderRadius: 10,
                border: "none",
                transition: "all 0.2s",
              }}
              onMouseEnter={(e) => {
                if(extractedOrder?.invoice) return;
                e.currentTarget.style.backgroundColor = "#2a2a44";
                e.currentTarget.style.transform = "translateY(-1px)";
                e.currentTarget.style.boxShadow =
            ...
            >
              <FileText size={16} />
              {extractedOrder?.invoice ? "Download Invoice" : "Generate GST Invoice"}
            </button>
            <button
      ...
      {editingOrder && extractedOrder && (
        <EditOrderModal
          order={extractedOrder}
          onClose={() => setEditingOrder(false)}
          onSave={async (updates) => {
            try {
              const updated = await client.editOrder(extractedOrder.id, updates);
