...
export function useOrders(limit = 5000, offset = 0) {
  const client = useApiClient();
  const res = useFetch<Order[]>(() => client.getOrders(limit, offset), [limit, offset]);
  
  useEffect(() => {
    const fn = () => res.refetch();
    window.addEventListener("bizchat:refresh_orders", fn);
    return () => window.removeEventListener("bizchat:refresh_orders", fn);
  }, [res.refetch]);
...
export function useStats() {
  const client = useApiClient();
  const res = useFetch<Stats>(() => client.getStats(), []);
